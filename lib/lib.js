    ...
    window.lib.noteClickHandler = function(note) {
        var noteID = note.id;
        var n = noteNames[noteID];
        window.lib.currentNote = n;

        document.getElementById("tbNoteOn").classList.add("tbSeqBtnOn");
        document.getElementById("tbNoteOff").classList.remove("tbSeqBtnOn");
    ...
    */
    ...
    // Sound function Object and prototype methods used for drum samples
    function sound(url, gain)
    {
        this.url = url;
        this.gain = gain;
        this.gain.gain.value = 0.9; // set to default value
    }

    // getters/setters for variables you want to retrieve more than once
    ...
    sound.prototype.play =
        function(time)
        {
            ...
            connectNodes(this.source, gain, trCompressor, trWaveShaper, trDelay, trGain);
